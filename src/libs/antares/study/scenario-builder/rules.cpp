...
void Rules::saveToINIFile(Yuni::IO::File::Stream& file) const
{
    ...
    if (pAreaCount)
    {
        ...
        // hydroEnergyCredits
        hydroEnergyCredits.saveToINIFile(study_, file);
        // wind
...
bool Rules::reset()
{
    ...
    solar.reset(study_);
    hydro.reset(study_);
    hydroEnergyCredits.reset(study_);
    wind.reset(study_);
...
bool Rules::readHydroEnergyCredits(const AreaName::Vector& splitKey, String value, bool updaterMode)
{
    const uint year = splitKey[2].to<uint>();
    const AreaName& areaname = splitKey[1];
    ...
    uint val = fromStringToTSnumber(value);
    hydroEnergyCredits.set(area->index, year, val);
    return true;
}
...
bool Rules::readLine(const AreaName::Vector& splitKey, String value, bool updaterMode = false)
{
    ...
    if (kind_of_scenario == "t")
        ...
        return readHydroEnergyCredits(splitKey, value, updaterMode);
