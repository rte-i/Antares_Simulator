...
double HydroManagement::prepareMonthlyTargetGenerationsForCluster(
  Data::Area& area,
  PerArea& data,
  uint clusterIndex)
{
    ...
    if (not Math::Zero(monthlyMaxDemand))
    {
        ...
        for (uint realmonth = 0; realmonth != 12; ++realmonth)
        {
            ...
            coeff
              += Math::Power(data.MLE[realmonth] / monthlyMaxDemand,
                             area.hydrocluster.clusters.at(clusterIndex)->intermonthlyBreakdown);
        }
...
void HydroManagement::prepareMonthlyOptimalGenerationsForCluster(double* random_reservoir_level,
                                                                 uint y,
                                                                 uint numSpace)
{
    uint indexArea = 0;
          ...
          area.hydrocluster.list.each(
            [&](const Data::HydroclusterCluster& cluster)
            {
                ...
                int initReservoirLvlMonth
                  = cluster.initializeReservoirLevelDate;
                ...
                if (cluster.reservoirManagement)
                {
                    ...
                    double totalInflowsYear = prepareMonthlyTargetGenerationsForCluster(area, data, cluster.index);
                    assert(totalInflowsYear >= 0.);
                ...
                {
                    auto& reservoirLevel = cluster.reservoirLevel[Data::PartHydro::average];
                ...
                for (uint realmonth = 0; realmonth != 12; ++realmonth)
                {
                    assert(!Math::NaN(data.MOG[realmonth]) && "nan value detected for MOG");
                    assert(!Math::NaN(data.MOL[realmonth]) && "nan value detected for MOL");
                    assert(!Math::Infinite(data.MOG[realmonth])
                           && "infinite value detected for MOG");
                    assert(!Math::Infinite(data.MOL[realmonth])
                ...
                {
                    String folder;
                    folder << study.folderOutput << SEP << "debug" << SEP << "solver" << SEP
                           << (1 + y);
                    if (IO::Directory::Create(folder))
                    {
                        String filename = folder;
                        ...
                        if (file.openRW(filename))
                        {
                            if (cluster.reservoirManagement)
                                ...
                                file
                                  << "Initial Reservoir Level : unrelevant (no reservoir mgmt)\n";
                            file << "\n";

                            auto writeSolutionCost
                              = [&file](const std::string& caption, double cost)
                            {
                                std::stringstream stream;
                                stream << caption << std::fixed << std::setprecision(13) << cost;
                                std::string cs = stream.str();
                            ...
                            for (uint month = 0; month != 12; ++month)
                            {
                                ...
                                uint firstDay
                                  = study.calendar.months[simulationMonth].daysYear.first;
