...
void SIM_AllocationProblemeHoraireAdequation()
{
    ...
    ValeursAnnuellesAdequation
      = (VALEURS_ANNUELLES**)MemAlloc(study.areas.size() * sizeof(VALEURS_ANNUELLES*));

    for (i = 0; i < (long)study.areas.size(); i++)
    {
        ValeursAnnuellesAdequation[i] = (VALEURS_ANNUELLES*)MemAlloc(sizeof(VALEURS_ANNUELLES));

        ValeursAnnuellesAdequation[i]->Consommation
          = (double*)MemAlloc(study.runtime->nbHoursPerYear * sizeof(double));
        ValeursAnnuellesAdequation[i]->PuissanceThermiqueCumulee
          = (double*)MemAlloc(study.runtime->nbHoursPerYear * sizeof(double));
        ValeursAnnuellesAdequation[i]->Reserve
          = (double*)MemAlloc(study.runtime->nbHoursPerYear * sizeof(double));
    }
    ...
    ProblemeHoraireAdequation.PointeurSurIndices
      = (long*)MemAlloc((study.areas.size() + 3) * sizeof(long));
    ProblemeHoraireAdequation.RangDansVoisinage
      = (long*)MemAlloc(ProblemeHoraireAdequation.NombreDElementsChainage * sizeof(long));
    ProblemeHoraireAdequation.QuellesNTCPrendre
      = (double*)MemAlloc(ProblemeHoraireAdequation.NombreDElementsChainage * sizeof(double));
    ProblemeHoraireAdequation.ValeursEffectivesNTC
      = (double*)MemAlloc(ProblemeHoraireAdequation.NombreDElementsChainage * sizeof(double));
    ProblemeHoraireAdequation.FluxDeSecours
      = (double*)MemAlloc(ProblemeHoraireAdequation.NombreDElementsChainage * sizeof(double));
    ProblemeHoraireAdequation.NTCDeReference
      = (double*)MemAlloc(ProblemeHoraireAdequation.NombreDElementsChainage * sizeof(double));
    ...
    ProblemeHoraireAdequation.DonneesParPays
      = (DONNEES_ADEQUATION**)MemAlloc(study.areas.size() * sizeof(DONNEES_ADEQUATION*));
    ProblemeHoraireAdequation.ResultatsParPays = (RESULTATS_HORAIRES_ADEQUATION**)MemAlloc(
      study.areas.size() * sizeof(RESULTATS_HORAIRES_ADEQUATION*));

    for (i = 0; i < (long)study.areas.size(); i++)
    {
        ProblemeHoraireAdequation.DonneesParPays[i]
          = (DONNEES_ADEQUATION*)MemAlloc(sizeof(DONNEES_ADEQUATION));
        ProblemeHoraireAdequation.ResultatsParPays[i]
          = (RESULTATS_HORAIRES_ADEQUATION*)MemAlloc(sizeof(RESULTATS_HORAIRES_ADEQUATION));
    }

    memset(ProblemeHoraireAdequation.IndicesDesVoisins,
           0,
           ProblemeHoraireAdequation.NombreDElementsChainage * sizeof(long));
    memset(
      ProblemeHoraireAdequation.PointeurSurIndices, 0, (study.areas.size() + 3) * sizeof(long));
    memset(ProblemeHoraireAdequation.RangDansVoisinage,
           0,
           ProblemeHoraireAdequation.NombreDElementsChainage * sizeof(long));
    memset(ProblemeHoraireAdequation.QuellesNTCPrendre,
           0,
...
void SIM_InitialisationChainagePourAdequation()
{
    ...
    for (i = 0; i < (long)study.runtime->interconnectionsCount; i++)
    {
        ...
        MatriceDuReseauAugmente[1 + lnk->from->index]->ParLigne[1 + lnk->with->index]
          = (double)(i + 1);
        MatriceDuReseauAugmente[1 + lnk->with->index]->ParLigne[1 + lnk->from->index]
          = (double)(-i - 1);
    }
    ...
    for (i = 0; i < (long)nbAreasPlus2; i++)
    {
        ...
        for (j = 0; j < (long)nbAreasPlus2; j++)
        {
            ...
            {
                NombreDeVoisins[i] += 1;
                assert((uint)(ProblemeHoraireAdequation.PointeurSurIndices[i] + k)
                       < (uint)ProblemeHoraireAdequation.NombreDElementsChainage);
                ProblemeHoraireAdequation
                  .IndicesDesVoisins[ProblemeHoraireAdequation.PointeurSurIndices[i] + k]
                  = j;
                AdjacentTemporaire[j]->ParLigne[i] = (double)k;
                k++;
            }
        }
        ProblemeHoraireAdequation.PointeurSurIndices[i + 1]
          = ProblemeHoraireAdequation.PointeurSurIndices[i] + NombreDeVoisins[i];
    }

    for (i = 0; i < (long)nbAreasPlus2; i++)
    {
        for (j = ProblemeHoraireAdequation.PointeurSurIndices[i];
             j < ProblemeHoraireAdequation.PointeurSurIndices[i + 1];
             j++)
        {
            ...
            ProblemeHoraireAdequation.RangDansVoisinage[j]
              = (long)(AdjacentTemporaire[i]
                         ->ParLigne[ProblemeHoraireAdequation.IndicesDesVoisins[j]]);
            ProblemeHoraireAdequation.QuellesNTCPrendre[j]
              = MatriceDuReseauAugmente[i]
                  ->ParLigne[ProblemeHoraireAdequation.IndicesDesVoisins[j]];
        }
    }
...
void SIM_RenseignementValeursPourTouteLAnnee(const Antares::Data::Study& study, uint numSpace) // SIM_InformationValuesForAnyYear
{
    ...
    for (uint i = 0; i != study.areas.size(); ++i)
    {
        ...
        for (uint h = startTime; h < finalHour; ++h)
        {
            ...
            Pt.Consommation[h]
              = +scratchpad.ts.load[tsIndex.Consommation][h] + primaryReserve[h]
                - scratchpad.ts.wind[tsIndex.Eolien][h] - scratchpad.miscGenSum[h]
...
void SIM_RenseignementProblemeHoraireAdequation(uint Heure)
{
    ...
    memset(ProblemeHoraireAdequation.ValeursEffectivesNTC,
           0,
           ProblemeHoraireAdequation.NombreDElementsChainage * sizeof(double));
...
void SIM_CalculFlotHoraireAdequation()
{
    ...
    {
        for (uint i = 0; i < study.areas.size(); i++)
            ProblemeHoraireAdequation.ResultatsParPays[i]->MargeAvecReseau
              = ProblemeHoraireAdequation.ResultatsParPays[i]->MargeHorsReseau;
    }
}
