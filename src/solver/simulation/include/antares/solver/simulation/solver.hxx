    ...
    for (ity = years.begin(); ity != years.end(); ++ity)
    {
        ...
        study.areas.each(
            ...
            Data::Area& area)
          {
              ...
              uint32_t seriesIndexMin = area.hydro.series->minDailyReservoirLevels.getSeriesIndex(
                y);
              const auto& min = area.hydro.series->minDailyReservoirLevels
                                  .timeSeries[seriesIndexMin];

              uint32_t seriesIndexMax = area.hydro.series->maxDailyReservoirLevels.getSeriesIndex(
                y);
              const auto& max = area.hydro.series->maxDailyReservoirLevels
                                  .timeSeries[seriesIndexMax];

              uint32_t seriesIndexAvg = area.hydro.series->avgDailyReservoirLevels.getSeriesIndex(
                y);
